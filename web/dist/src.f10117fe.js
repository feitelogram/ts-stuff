...
function (_super) {
  ...
  UserShow.prototype.template = function () {
    var age = this.model.get('age') || 100;
  ...
  Collection.prototype.fetch = function () {
    ...
    axios_1.default.get(this.rootUrl).then(function (resp) {
      resp.data.forEach(function (val) {
        var model = _this.deserialize(val);
...
function () {
  ...
  Model.prototype.save = function () {
    ...
    this.sync.save(attrs).then(function (resp) {
      _this.trigger('save');
    }).catch(function () {
...
function (_super) {
  ...
  User.buildUser = function (attrs) {
    return new User(new Attributes_1.Attributes(attrs), new Eventing_1.Eventing(), new Sync_1.ApiSync(rootUrl));
  };
...
var col = new Collection_1.Collection('http://localhost:3000/users', function (json) {
  return User_1.User.buildUser(json);
});
col.on('change', function () {
  var root = document.querySelector('#root');
  new UserList_1.UserList(root, col).render();
});
col.fetch();
